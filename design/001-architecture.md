...
| | |
| --- | --- |
| 状态 | 硬分叉（不再 merge penguin-harness）；M0–M2 库已验证；**开放里程碑 M3/M5 已于 2026-08-18 撤出规划**（产品方向另议，§8 注记） |
| 日期 | 2026-08-12 |
...
### 7.6 无显示器机器上的扩展模式 ✅ 已打通（2026-08-12；产品默认于 2026-08-19 修订）

`scripts/dev-browser.sh`（`start` / `stop` / `status`，幂等）。

本节记录的是当时验证扩展链路的历史背景；“扩展不是可选项”的产品判断已经废止。当前 Desktop 默认使用 IAB，IAB 有独立且可持久化的登录态；只有需要复用用户既有 Chrome 登录、SSO、证书或已打开标签页时，才由用户在每个对话的 Browser 菜单中明确选择扩展。所选后端在任务中锁定，不允许 agent 静默换用另一套身份。

此前判断“本机做不了扩展模式”仍然是错的——障碍不是 Chrome（`penguin-browser browser install` 早就装好了一个），而是**显示环境**：加载未打包扩展需要有窗口的浏览器，`--headless` 没有窗口。而 Xvfb 提供的虚拟显示就够了：Chrome 往里渲染，扩展的 service worker 正常运行，像在桌面上一样连上 relay。
...
> **2026-08-18 注记**：未完成的开放里程碑（M3 演示场景闭环、M5 机票）已从规划中撤除——产品接下来的方向留待专门讨论，不再预排。下表与依赖图**保留为历史记录**，其中 M3/M5 的行不再是承诺。
...
4. **扩展模式的授权摩擦**——授权边界已于 2026-08-19 定稿：Browser 菜单选择 Chrome，允许该对话创建自己的任务 tab；接管用户已经打开的 tab，仍需在目标页点击扩展图标。`PENGUIN_BROWSER_AUTO_ENABLE` 因而保留。尚需通过人工测试验证安装引导、多 Profile 选择与跨平台并行的实际感受。
