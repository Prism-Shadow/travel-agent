...
1. **Cloud browser sessions via Browser Use** — spin up stealth Chromium VMs in the cloud with `penguin-browser session new --browser cloud`. Cloud browsers support residential proxies (`--proxy us`, `--proxy de`), custom proxies (`--custom-proxy host:port`), and configurable timeouts (`--timeout 120`). Idle sessions auto-disconnect after 10 minutes.
   ...
   ```bash
   penguin-browser cloud login       # authenticate via device flow
   penguin-browser cloud status      # list active cloud VMs
   penguin-browser cloud subscribe   # open subscription page
   penguin-browser cloud live        # open live browser view
   ```
...
2. **Headless browser mode** — run without the extension or a visible browser:
   ```bash
   penguin-browser browser install                    # download Chrome for Testing
   penguin-browser session new --browser headless      # launch headless Chrome
...
3. **API key authentication for cloud browsers** — skip the interactive device flow in CI and headless environments:
   ```bash
   export PENGUIN_BROWSER_API_KEY=pw_xxxxx
   penguin-browser session new --browser cloud
   ```

   Create and revoke keys at https://private-project.invalid/dashboard. Authentication priority: `PENGUIN_BROWSER_API_KEY` env var, then `PENGUIN_BROWSER_CLOUD_TOKEN` env var, then `~/.penguin-browser/auth.json`.
...
5. **Fixed cloud billing race conditions** — concurrent `penguin-browser session new --browser cloud` requests now use durable per-org slot claims, preventing over-provisioning beyond the subscribed session quantity. Stripe Checkout also verifies Stripe directly before creating subscriptions, avoiding duplicates while webhook delivery is pending.

6. **Fixed `penguin-browser cloud login`** — the CLI now uses Better Auth's current device authorization endpoints (`/api/auth/device/code`, `/api/auth/device/token`) so cloud browsers appear after approving the login.

## 0.3.1

1. **Auto-page creation enabled by default** — MCP and CLI sessions now automatically create a blank Penguin Browser-enabled tab when no targets are available, so agents can start working immediately without manual tab setup. Set `PENGUIN_BROWSER_AUTO_ENABLE=false` to disable.

## 0.3.0

1. **New `sinceLastCall` option for `getLatestLogs()`** — inspect browser logs after every action without seeing duplicate messages:
   ```bash
   penguin-browser -s 1 -e 'console.log(await getLatestLogs({ page, sinceLastCall: true }))'
   ```
...
2. **CDP logs now rotate automatically** — `~/.penguin-browser/cdp.jsonl` is capped at 10,000 entries by default to prevent unbounded disk growth. Set `PENGUIN_BROWSER_CDP_LOG_MAX_ENTRIES` to tune the cap. Rotation keeps the newest half of the log and writes through an atomic temp-file rename to avoid corrupting the JSONL file.
...
4. **CLI-created sessions auto-open pages more reliably** — `penguin-browser session new` can auto-create an initial extension tab even when the shared relay was originally started by MCP. This avoids `No Playwright pages are available` after all enabled tabs have closed.
...
6. **Concurrent relay startup no longer crashes on port races** — simultaneous CLI and MCP commands now deduplicate startup work and treat a competing process winning port `19989` as a clean handoff instead of surfacing `EADDRINUSE`.
7. **Clearer multi-browser names in `penguin-browser session new`** — browser lists now use full user-agent client hints when available, so Chromium-family browsers such as Chrome Canary can show a more specific name.
8. **Skill docs prefer `getLatestLogs()` for page diagnostics** — the generated Penguin Browser skill now tells agents to call `getLatestLogs({ page })` instead of adding manual console listeners that miss errors emitted before listener setup.

## 0.2.0

1. **New `-f/--file` flag** — execute JavaScript from a file instead of inline `-e` strings:
   ```bash
   penguin-browser -s 1 -f script.js
   ```
...
3. **Performance profiling guide** — new generated `performance-profiling.md` resource covering TTFB, FCP, LCP, CLS measurement, heavy request detection, and interactivity blockers with concrete Penguin Browser + CDP snippets. Also links to `profano` for deeper `.cpuprofile` analysis.
4. **Shell tab completions** — `penguin-browser` now supports shell completions via goke. Run the completion setup for your shell to get tab completion on commands and flags.
...
6. **`--token` works on every remote subcommand** — `session new`, `session list`, `session delete`, `session reset`, and `browser list` all forward `Authorization: Bearer …` to the relay's `/cli/*` endpoints. Previously only `penguin-browser -e` sent the token. Thanks to @ivanleomk for the original fix.
...
1. **New in-page toolbar with pin mode** — every attached tab now gets a floating toolbar you can use to pin elements directly from the page. Pinning copies a natural-language prompt plus the exact `penguin-browser -e '…'` code needed to inspect that element later, so pasted prompts are immediately useful to an agent instead of just exposing a fragile DOM handle.
2. **Always-on ghost cursor with better motion** — the cursor overlay now appears on every Penguin Browser-attached tab, survives hard navigations, uses smoother move/press animation timing, and fades away after 5 seconds of idle time so manual browsing stays uncluttered. The next Playwright-driven mouse action brings it back instantly.
3. **`penguin-browser --help` and `penguin-browser serve --help` work on clean installs again** — browser-launch code is now lazy-loaded only when `penguin-browser browser start` actually runs, so generic CLI entrypoints no longer fail early on unrelated browser-install dependencies.
...
1. **Executor logs new pages instead of unreachable popups**. Previously, when a page opened another via `window.open` or `target="_blank"`, the executor emitted `[WARNING] Popup window detected ... cannot be controlled by penguin-browser` and told the agent to retry. Paired with the extension 0.0.80 change (popups are auto-relocated to tabs in the source tab's window), the warning is now `[WARNING] New page opened from current page (index N, initial url: ...)` pointing the agent at the new tab to interact with it.
2. **Minimum extension version check**. If the user has an outdated Penguin Browser extension (< 0.0.80) that doesn't support popup relocation, the CLI/MCP now emits a warning telling them to update the extension via `chrome://extensions`. The warning is also enqueued into the MCP agent's warning stream so the agent knows why popup behavior is broken.
...
3. **`PENGUIN_BROWSER_DIRECT` only accepts `'1'`** — removed `'auto'` and `'true'` aliases. The standard boolean env var pattern (`PENGUIN_BROWSER_DIRECT=1`) is the only accepted value for auto-discovery. Explicit `ws://` endpoints still work as before.

## 0.0.101

1. **Chrome 136+ direct CDP discovery** — `penguin-browser browser list` and `--direct` auto-discovery now detect Chrome instances where `/json/version` returns 404 (Chrome 136+ with `chrome://inspect` debugging). Previously these were silently ignored. Discovery uses HTTP-only probing and never triggers Chrome's approval dialog.
2. **`--direct` moved to `session new` only** — removed the `--direct` flag from the root command. For MCP, set `PENGUIN_BROWSER_DIRECT=1` env var instead.
...
1. **`resizeImageForAgent` now defaults to PNG** — previously defaulted to JPEG, which caused `image/png` vs `image/jpeg` mismatch errors when MCP clients assumed PNG. All images emitted by penguin-browser are now consistently PNG unless explicitly overridden with `format: 'jpeg'`.
...
1. **Direct CDP connection mode** — new `--direct` flag on `session new` connects to Chrome's built-in debugging WebSocket without needing the Penguin Browser extension. Works with any Chromium-based browser (Chrome, Brave, Ghost Browser, Arc, Edge, etc.) that has debugging enabled via `chrome://inspect/#remote-debugging` or `--remote-debugging-port`. Auto-discovers instances via DevToolsActivePort files and port scanning (9222-9229). Recording is unavailable in this mode.
2. **`penguin-browser browser list` command** — lists all Chrome/Chromium instances with debugging enabled, showing port, browser name, and profile info.
3. **MCP direct mode** — set `PENGUIN_BROWSER_DIRECT=1` or `PENGUIN_BROWSER_DIRECT=ws://...` env var to start the MCP server in direct CDP mode without a relay server.
...
1. **Document managed-browser recording permissions** — `penguin-browser browser start` now clearly reports that recording/tab-capture flags are enabled, and the skill docs now explain that `recording.start()` does not require a manual extension click when using the managed browser flow.

## 0.0.95

1. **Skip the welcome tab for bundled automation builds** — the Penguin Browser CLI now packages an extension build with `welcome.html` disabled on install, so fresh managed browser profiles do not waste a tab in headless and AVPS flows.

## 0.0.94

1. **Make `browser start` work from source checkouts** — runtime package path resolution now falls back to the local `penguin-browser/` package directory before using installed-package resolution, so `tsx penguin-browser/src/cli.ts browser start` works during development.
...
1. **Show searched browser paths on launch failures** — `penguin-browser browser start` now prints every Chrome for Testing / Chromium path it checked, making it much easier to debug autodiscovery issues on local machines and AVPS hosts.

## 0.0.92

1. **Add `penguin-browser browser start`** — the CLI can now launch a managed Chrome for Testing or Chromium instance with the bundled Penguin Browser extension preloaded, making fresh AVPS/VPS automation setups much easier.
2. **Bundle the extension into the npm package** — Penguin Browser now builds and ships an unpacked extension copy inside `dist/extension`, resolved at runtime from the installed package path so the CLI can side-load it without depending on a separate checkout.
...
1. **Show session cwd in `penguin-browser session list`** — the CLI session table now includes the working directory each session was created with, making it easier to tell similar sessions apart.
2. **Fix session cwd leakage across relay restarts** — relative `fs` paths in the sandbox now resolve from the cwd captured by `penguin-browser session new`, instead of whichever directory last launched the detached relay server.
...
- **Reduce false "extension disconnected" on relay restarts**: `penguin-browser session new` now waits longer for extension reconnect and adds a short polling grace window before failing, preventing transient post-restart races from surfacing as hard disconnect errors.
...
- **Ghost cursor overlay during recording**: Penguin Browser now auto-enables a smooth in-page ghost cursor when `startRecording()` is called, driven by `page.onMouseAction` callbacks from the Playwright fork so both `page.mouse.*` and `locator.click()` actions are visualized.
...
- **Remote access support**: `PENGUIN_BROWSER_HOST` now accepts full URLs (e.g., `https://x-tunnel.traforo.dev`) in addition to plain hostnames, enabling secure remote browser access through tunnels like traforo
- **WebSocket over HTTPS**: Automatically uses `wss://` protocol when connecting to HTTPS relay hosts
- **Remote access documentation**: Added comprehensive guide covering architecture, setup, use cases, and security model for remote Penguin Browser access
...
- **Fix relay startup EADDRINUSE timeouts**: If the relay port is already bound but `/version` is not responding, Penguin Browser now detects the listening PID(s), stops the existing process, and only then starts the relay (the 5s startup timeout now measures post-spawn readiness, not port cleanup time).
- **Harden port-kill implementation**: Replaced Penguin Browser's port killer with an implementation that mirrors `kill-port-process` (lsof/grep/awk/xargs on unix; taskkill on Windows) and includes the `xargs.stdout` pipe fix from upstream PR #199.
...
- **Fix "Cannot find module 'graceful-fs'" error**: Updated `@xmorse/playwright-core` to 1.59.3 which adds missing runtime dependencies (`graceful-fs`, `retry`, `signal-exit`) for clean `npx penguin-browser` installs (GitHub #45)

## 0.0.58

### Bug Fixes

- **Fix `bunx penguin-browser@latest` relay restarts**: Replaced `kill-port-process` with a vendored cross-platform port killer to avoid runtime crashes during version-mismatch restart flows.
...
- **CDP JSONL Logging**: Added structured CDP logging to `~/.penguin-browser/cdp.jsonl`
...
- **Fix log dir permissions on shared machines**: Move default log directory from `/tmp/penguin-browser` to `~/.penguin-browser` so each OS user gets their own directory. Fixes startup crash when `/tmp/penguin-browser` is owned by another user (#44).
...
- **CDP JSONL logging**: Added structured CDP logging to a JSONL file (`/tmp/penguin-browser/cdp.jsonl`) for debugging. Log all CDP messages with direction, timestamp, and source info. Use `jq` to analyze.
...
- **`penguin-browser skill` CLI command**: New command that prints full MCP instructions to stdout, useful for agents that need up-to-date documentation without relying on MCP resources
...
- **First extension keeps connection**: When multiple Penguin Browser extensions are installed (e.g., dev and prod), the first one with active tabs now keeps the connection instead of being replaced by newer connections. Idle extensions (no tabs) can still be replaced.
...
- **Session management CLI commands**: New CLI commands for managing relay sessions (`penguin-browser sessions list`, `penguin-browser sessions kill`)
- **Eval CLI flag**: New `-e/--eval` CLI flag for quick code execution from command line
- **Auto-enable environment variable**: CLI now passes `PENGUIN_BROWSER_AUTO_ENABLE` when starting relay server
...
- **Clearer error messages**: Improved error messages when another Penguin Browser extension connects, making it easier to diagnose connection issues
...
- **CDP discovery endpoints**: Added standard Chrome DevTools Protocol HTTP discovery endpoints
  ...
  - Allows `chromium.connectOverCDP('http://127.0.0.1:19989')` without needing to call `getCdpUrl` first
...
- **Extension fix**: Query penguin-browser tab group by title instead of caching ID, fixing stale group issues after debugger detach/reattach
...
- **Auto-create initial tab**: When `PENGUIN_BROWSER_AUTO_ENABLE` env var is set, automatically creates an about:blank tab when Playwright connects and no tabs exist
...
- **Hosted resources on private-project.invalid**: Resources now use `https://private-project.invalid/resources/*.md` URLs instead of `penguin-browser://` custom URIs
...
- **Added `styles-api` resource**: New MCP resource (`penguin-browser://styles-api`) with types and examples for `getStylesForLocator` CSS inspection API
- **Reduced prompt context**: Simplified prompt.md to reference resources (`penguin-browser://debugger-api`, `penguin-browser://editor-api`, `penguin-browser://styles-api`) instead of inline documentation
...
- **Improved debug message clarity**: Changed log file path hint to specify "internal penguin-browser errors" for better guidance
...
- **Improved error debugging**: Log file path now included in error messages and tool description. Log file writes to OS temp directory by default (`PENGUIN_BROWSER_LOG_PATH` env var to override)
...
- Added `Penguin Browser.activateTab` CDP command support in relay server
...
- startPenguinBrowser now simply throws an error for invalid emails
...
- Modified startPenguinBrowser to accept optional emailProfile parameter
...
- Integrated Chrome launch via startPenguinBrowser from penguin-browser.ts
