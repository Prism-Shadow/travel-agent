...
import { enableGhostCursor, applyGhostCursorMouseAction, disableGhostCursor } from '../src/cursor/ghost-cursor.js'
import {
  setupTestContext,
  cleanupTestContext,
...
describe('onMouseAction callback', () => {
  ...
  it('should animate ghost cursor from onMouseAction callback', async () => {
    ...
    try {
      ...
      const cursorState = await pageForTest.evaluate(() => {
        const cursorElement = document.getElementById('__penguinBrowser_ghost_cursor__')
        if (!cursorElement) {
      ...
      const hasGhostCursor = await pageForTest.evaluate(() => {
        return Boolean(document.getElementById('__penguinBrowser_ghost_cursor__'))
      })
  ...
  it('should inject ghost cursor into attached tabs without explicit enable', async () => {
    ...
    const cursorPresent = await page.evaluate(() => {
      return {
        apiPresent: Boolean((globalThis as any).__penguinBrowserGhostCursor),
        elementPresent: Boolean(document.getElementById('__penguinBrowser_ghost_cursor__')),
      }
    })
