...
import type { AriaSnapshotNode } from '../src/page/aria-snapshot.js'
...
describe('Snapshot & Screenshot Tests', () => {
  ...
  it('should get aria ref for locator using getAriaSnapshot', async () => {
    ...
    const { getAriaSnapshot } = await import('../src/page/aria-snapshot.js')
  ...
  it.skipIf(process.env.CI)('should show aria ref labels on real pages and save screenshots', async () => {
    ...
    const { showAriaRefLabels, hideAriaRefLabels } = await import('../src/page/aria-snapshot.js')

    // Keep generated QA screenshots inside this test run's temporary profile.
    // Writing them into src/assets mutates repository fixtures on every run.
    const assetsDir = path.join(testCtx!.userDataDir, 'qa-artifacts')
    ...
    for (const { name, url, page } of pages) {
      ...
      const labelsAfterHide = await withTimeout(
        `verifyHide(${name})`,
        async () => {
          return await cdpPage.evaluate(() => document.getElementById('__penguinBrowser_labels__'))
        },
        10000,
      )
