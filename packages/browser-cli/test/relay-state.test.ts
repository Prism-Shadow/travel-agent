...
import type { Protocol } from '../src/relay/cdp-types.js'
import * as relayState from '../src/relay/relay-state.js'
...
describe('updateTargetUrl', () => {
  ...
  test('does not replace a page target URL when a child iframe navigates', () => {
    let state = stateWithExtension('ext-1')
    state = relayState.addTarget(state, {
      extensionId: 'ext-1',
      sessionId: 'pw-tab-1',
      targetId: 'page-frame',
      targetInfo: makeTargetInfo({ targetId: 'page-frame', url: 'about:blank' }),
    })

    const after = relayState.updateTargetUrlForFrame(state, {
      extensionId: 'ext-1',
      sessionId: 'pw-tab-1',
      frameId: 'child-frame',
      url: 'http://localhost:8044/docs',
    })

    expect(after).toBe(state)
    expect(after.extensions.get('ext-1')!.connectedTargets.get('pw-tab-1')!.targetInfo.url).toBe('about:blank')
  })

  test('updates a page target URL when its main frame navigates', () => {
    let state = stateWithExtension('ext-1')
    state = relayState.addTarget(state, {
      extensionId: 'ext-1',
      sessionId: 'pw-tab-1',
    ...
    expect(after.extensions.get('ext-1')!.connectedTargets.get('pw-tab-1')!.targetInfo.url).toBe('https://example.com/')
  })
})
...
describe('Host header validation (DNS rebinding protection)', () => {
  ...
  beforeAll(async () => {
    const { startPenguinBrowserCDPRelayServer } = await import('../src/relay/cdp-relay.js')
    server = await startPenguinBrowserCDPRelayServer({ port: TEST_PORT })
  })
