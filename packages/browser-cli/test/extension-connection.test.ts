...
import { getExtensionsStatus } from '../src/relay/relay-client.js'
...
describe('Extension Connection Tests', () => {
  ...
  it('serializes a rapid manual detach followed by reattach on the same tab', async () => {
    const browserContext = getBrowserContext()
    const serviceWorker = await getExtensionServiceWorker(browserContext)
    const page = await browserContext.newPage()
    const testUrl = 'https://example.com/serialized-detach-reattach'
    await page.goto(testUrl, { waitUntil: 'domcontentloaded' })
    await page.bringToFront()

    const initial = await serviceWorker.evaluate(async () => {
      return await globalThis.toggleExtensionForActiveTab()
    })
    expect(initial.isConnected).toBe(true)

    const raceResult = await serviceWorker.evaluate(async () => {
      const activeTabs = await chrome.tabs.query({ active: true, currentWindow: true })
      const tabId = activeTabs[0]?.id
      if (!tabId) throw new Error('No active tab')

      const debuggerApi = chrome.debugger as any
      const originalAttach = debuggerApi.attach
      const originalDetach = debuggerApi.detach
      const events: string[] = []
      let releaseDetach!: () => void
      let signalDetachStarted!: () => void
      ...
      debuggerApi.detach = async (debuggee: chrome.debugger.Debuggee) => {
        if (debuggee.tabId !== tabId) {
          return await originalDetach.call(debuggerApi, debuggee)
        }
        events.push('detach:start')
        signalDetachStarted()
        await detachGate
        const result = await originalDetach.call(debuggerApi, debuggee)
        events.push('detach:end')
        return result
      }
      debuggerApi.attach = async (debuggee: chrome.debugger.Debuggee, version: string) => {
        if (debuggee.tabId === tabId) events.push('attach:start')
        const result = await originalAttach.call(debuggerApi, debuggee, version)
        if (debuggee.tabId === tabId) events.push('attach:end')
        return result
      }

      try {
        const disconnect = globalThis.toggleExtensionForActiveTab()
        await detachStarted
        const reconnect = globalThis.toggleExtensionForActiveTab()

        await new Promise((resolve) => setTimeout(resolve, 50))
        const attachStartedBeforeDetachCompleted = events.includes('attach:start')
        releaseDetach()
        ...
        return {
          events,
          attachStartedBeforeDetachCompleted,
          finalTabState: state.tabs.get(tabId)?.state,
        }
      } finally {
        debuggerApi.attach = originalAttach
        debuggerApi.detach = originalDetach
      }
    })

    expect(raceResult.attachStartedBeforeDetachCompleted).toBe(false)
    expect(raceResult.events).toEqual(['detach:start', 'detach:end', 'attach:start', 'attach:end'])
    expect(raceResult.finalTabState).toBe('connected')

    const directBrowser = await chromium.connectOverCDP(getCdpUrl({ port: TEST_PORT }))
    const connectedPage = directBrowser
      .contexts()[0]
      .pages()
      .find((candidate) => candidate.url() === testUrl)
    expect(connectedPage).toBeDefined()
    expect(await connectedPage?.evaluate(() => 21 * 2)).toBe(42)
  ...
  it('should be able to reconnect after disconnecting everything', async () => {
    ...
    // 3. The default auto-enable behavior should recover by creating a new
    // authorized about:blank page after all existing tabs are detached.
    const afterDisconnect = await client.callTool({
      ...
      arguments: {
        code: js`
          ...
          return { pagesCount: pages.length, urls: pages.map(p => p.url()) };
        `,
      },
    })
  ...
  it('should keep an active browser connected when another Chromium context starts', async () => {
    ...
    const extensionPath = path.resolve(process.cwd(), '../browser-extension', `dist-${TEST_PORT}`)
    ...
    try {
      ...
      expect(
        statusSnapshots.every((snapshot) => {
