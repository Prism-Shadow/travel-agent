...
import { chromium, Page } from '@xmorse/playwright-core'
import fs from 'node:fs'
import path from 'node:path'
import { getAriaSnapshot } from '../src/page/aria-snapshot.js'
...
const SHOULD_DUMP_AX = process.env.PENGUIN_BROWSER_DUMP_AX === '1'
...
describe('aria-snapshot', () => {
  ...
  it('prosemirror editor - contenteditable appears in snapshot and is interactable', async () => {
    ...
    await page.keyboard.type('Hello from penguin-browser test!')

    const content = await editor.innerText()
    expect(content).toContain('Hello from penguin-browser test!')

    // Take full snapshot again and verify the new content is visible
    const { snapshot: afterSnapshot } = await getAriaSnapshot({ page })
    expect(afterSnapshot).toContain('Hello from penguin-browser test')
  }, 30000)
...
<body>
  ...
  it('snapshots a frame through its owning page session without overwriting the page target URL', async () => {
    const htmlServer = await createHtmlServer({
      htmlByPath: {
        '/unrelated': `<!doctype html><html><body><h1>Unrelated page</h1></body></html>`,
      },
    })
    const serviceWorker = await getExtensionServiceWorker(ctx.browserContext)
    const unrelatedPage = await ctx.browserContext.newPage()
    let relayBrowser: Awaited<ReturnType<typeof chromium.connectOverCDP>> | null = null

    try {
      await page.goto('about:blank')
      await page.setContent(
        `<iframe title="regression-frame" srcdoc="<h1>Frame snapshot regression</h1><button>Frame action</button>"></iframe>`,
      )
      await page.frameLocator('iframe[title="regression-frame"]').getByRole('heading').waitFor()

      await unrelatedPage.goto(`${htmlServer.baseUrl}/unrelated`, { waitUntil: 'domcontentloaded' })
      await unrelatedPage.bringToFront()
      await serviceWorker.evaluate(async () => {
        await (globalThis as any).toggleExtensionForActiveTab()
      })

      relayBrowser = await chromium.connectOverCDP(getCdpUrl({ port: TEST_PORT }))
      const relayPages = relayBrowser.contexts()[0].pages()
      const frameOwnerPage = relayPages.find((candidate) => {
        return candidate.frames().some((candidateFrame) => candidateFrame.url() === 'about:srcdoc')
      })
      const relayUnrelatedPage = relayPages.find((candidate) => candidate.url() === `${htmlServer.baseUrl}/unrelated`)
      expect(frameOwnerPage).toBeDefined()
      expect(relayUnrelatedPage).toBeDefined()

      const frame = frameOwnerPage!.frames().find((candidate) => candidate.url() === 'about:srcdoc')
      expect(frame).toBeDefined()

      const ownerCdp = await getCDPSessionForPage({ page: frameOwnerPage! })
      const { targetInfos } = await ownerCdp.send('Target.getTargets')
      const ownerTarget = targetInfos.find((target) => target.targetId === frameOwnerPage!.mainFrame().frameId())
      expect(ownerTarget?.url).toBe('about:blank')

      // Reproduces snapshot({ frame }) when the executor's default page is a
      // different connected tab. The frame's owner must select the CDP session.
      const { snapshot } = await getAriaSnapshot({ page: relayUnrelatedPage!, frame: frame! })
      expect(snapshot).toContain('Frame snapshot regression')
      expect(snapshot).toContain('Frame action')
      await ownerCdp.detach()
    } finally {
      if (relayBrowser) {
        await safeCloseCDPBrowser(relayBrowser)
      }
