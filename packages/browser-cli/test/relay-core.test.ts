...
import { getCdpUrl, LOG_CDP_FILE_PATH } from '../src/shared/utils.js'
  ...
  <body>
    ...
    <script>
      ...
      button.addEventListener('click', () => {
        const blob = new Blob(['penguin-browser-download-test'], { type: 'text/plain' });
        const url = URL.createObjectURL(blob);
        const anchor = document.createElement('a');
        anchor.href = url;
        anchor.download = 'penguin-browser-download-test.txt';
    ...
    expect(downloadResult[0]).toMatchInlineSnapshot(`
      {
        "suggestedFilename": "penguin-browser-download-test.txt",
        "timedOut": false,
      }
    `)
...
<html>
  ...
  // Regression test for https://private-project.invalid/repository/issues/66.
  // Playwright sends context cookies through root-session Storage commands while
  // route.fetch() builds its API request. Extension mode must safely route those
  // commands through an authorized page in the same browser context.
  it('should support page.route fetch and context cookies in extension mode (issue #66)', async () => {
    const browserContext = getBrowserContext()
    const unrelatedCookieName = 'issue66-unrelated'
    const server = await createSimpleServer({
      routes: {
    ...
    try {
      ...
      const result = await client.callTool({
        ...
        arguments: {
          code: js`
            ...
            const cookiesAfterSet = await context.cookies('${server.baseUrl}');

            let routeFetchError = null;
            let routeFetchStatus = null;
            await newPage.route('**/api/**', async (route) => {
              try {
                const response = await route.fetch();
                routeFetchStatus = response.status();
            ...
            const pageResponseBody = await newPage.evaluate(async () => {
              return await fetch('/api/data').then(response => response.text());
            });

            await context.clearCookies();
            const cookiesAfterClear = await context.cookies('${server.baseUrl}');
      ...
      expect(output).toContain('routeFetchError: null')
      expect(output).toContain('routeFetchStatus: 200')
      expect(output).toContain(`pageResponseBody: '{"ok":true}'`)
      expect(output).toContain("name: 'issue66'")
      expect(output).toContain("value: 'cookie-value'")
      expect(output).toContain('cookiesAfterClear: []')
      expect(output).not.toContain('No tab found for method Storage.')

      const unrelatedCookies = await browserContext.cookies('https://unrelated.invalid')
      expect(unrelatedCookies).toEqual(
      ...
      await browserContext.clearCookies({ name: unrelatedCookieName, domain: 'unrelated.invalid' })
      await server.close()
    }
  }, 30000)
  ...
  const snapshotTestCases = [
    ...
    },
    {
      ...
      waitForCode: js`
        await state.page.waitForFunction(() => document.title.toLowerCase().includes('shadcn'), undefined, { timeout: 15000 });
        await state.page.locator('body').waitFor({ state: 'visible', timeout: 10000 });
      `,
    },
  ]
    ...
    it(`should get accessibility snapshot of ${testCase.name}`, async () => {
      ...
      const interactiveData =
        typeof interactiveResult === 'object' && interactiveResult.content?.[0]?.text
          ...
          : interactiveResult
      // External pages are dynamic; verify semantic functionality rather than
      // brittle byte-for-byte page content.
      expect(interactiveResult.isError).toBeFalsy()
      expect(interactiveResult.content).toBeDefined()
      expect(typeof interactiveData).toBe('string')
      ...
      expect(typeof fullData).toBe('string')
      for (const expected of testCase.expectedContent) {
  ...
  it('should preserve system color scheme instead of forcing light mode', async () => {
    ...
    // Record the browser's actual scheme before the CDP connection. Extension
    // mode must preserve it rather than forcing either light or dark.
    ...
    const colorOutput = (result.content as any)[0]?.text || ''
    expect(colorOutput).toContain(`matchesDark: ${colorSchemeBefore === 'dark'}`)
    expect(colorOutput).toContain(`matchesLight: ${colorSchemeBefore === 'light'}`)
  ...
  it('should handle default page being closed and switch to another available page', async () => {
    ...
    // 7. Execute immediately, before Target.detachedFromTarget is guaranteed to
    // reach the MCP's separate Playwright client. This should NOT fail with a
    // "page closed" error.
  ...
  it('should show descriptive error when clicking a hidden element', async () => {
    ...
    expect(errorText).toContain('Element is not visible')
    expect(errorText).toContain('collapsed <details>')
    // Cleanup
    await client.callTool({
      name: 'execute',
  ...
  it('should show descriptive error when clicking an element covered by another', async () => {
    ...
    expect((result as any).isError).toBe(true)
    const errorText = (result as any).content[0].text
    expect(errorText).toContain('intercepts pointer events')
    expect(errorText).toContain('overlay')
    await client.callTool({
      name: 'execute',
      arguments: { code: js`await state.errorTestPage.close(); delete state.errorTestPage;` },
    })
  ...
  it('should show descriptive error when clicking a display:none element', async () => {
    ...
    const errorText = (result as any).content[0].text
    expect(errorText).toContain('Element is not visible')
    expect(errorText).toContain('hidden by CSS')
    await client.callTool({
      name: 'execute',
      arguments: { code: js`await state.errorTestPage.close(); delete state.errorTestPage;` },
    })
