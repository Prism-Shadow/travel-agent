...
the `startPenguinBrowserCDPRelayServer` closure:
...
(`startPenguinBrowserCDPRelayServer` return type, HTTP endpoints, WebSocket protocol)
...
top of the `startPenguinBrowserCDPRelayServer` closure, replacing the separate
