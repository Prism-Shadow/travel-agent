...
/**
 * Return Playwright's existing CDP session for an out-of-process iframe.
 * Same-process frames are part of the page session and return null.
 */
export async function getCDPSessionForFrame({ frame }: { frame: Frame }): Promise<PlaywrightCDPSessionAdapter | null> {
  try {
    const playwrightSession = await frame.page().context().getExistingCDPSession(frame)
    return new PlaywrightCDPSessionAdapter(playwrightSession)
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    if (message.includes('does not have a separate CDP session')) {
      return null
    }
    throw error
  }
