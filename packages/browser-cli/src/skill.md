...
- Avoid interference when multiple agents use penguin-browser simultaneously
...
```bash
penguin-browser session reset <sessionId>
```
...
export PENGUIN_BROWSER_TOKEN=MY_SECRET_TOKEN
penguin-browser session new
...
Penguin Browser can connect directly to a Chrome instance via the Chrome DevTools Protocol, bypassing the browser extension entirely. This is useful for:
...
- Use `penguin-browser browser start` (enables debugging automatically)
...
penguin-browser session new --direct

# Connect to a specific CDP endpoint (local or cloud browser provider)
penguin-browser session new --direct ws://localhost:9222/devtools/browser/...
penguin-browser session new --direct wss://xxx.cdp.browser-use.com

# Connect to a remote Chrome instance (host:port auto-resolves to ws://)
penguin-browser session new --direct 192.168.1.50:9222

# Then use the session normally
penguin-browser -s 1 -e "await page.goto('https://example.com')"
```

**MCP configuration** (for AI assistants): set the `PENGUIN_BROWSER_DIRECT` env var in your MCP client config. If the user provides a CDP URL (like `wss://xxx.cdp.browser-use.com`), use it as the value:
...
`PENGUIN_BROWSER_DIRECT` accepts:
...
penguin-browser browser install

# Launch headless Chrome and create a session
penguin-browser session new --browser headless

# Use the session normally
penguin-browser -s 1 -e "await page.goto('https://example.com')"
penguin-browser -s 1 -e "console.log(await snapshot({ page }))"
```

Multiple sessions reuse the same headless Chrome process. Recording is not available in headless mode.

If no Chrome binary is found, `penguin-browser session new --browser headless` will tell you to run `penguin-browser browser install` first to download Chrome for Testing.
...
penguin-browser cloud login

# Option 2: API key (for CI, VPS, headless — no browser needed)
# Requires a configured Penguin Browser cloud deployment. Create an API key in that deployment, then:
export PENGUIN_BROWSER_API_KEY=pw_xxxxx
```

```bash
# Check active cloud sessions
penguin-browser cloud status
...
penguin-browser session new --browser cloud --proxy us

# Use a different region
penguin-browser session new --browser cloud --proxy de

# Use a custom proxy
penguin-browser session new --browser cloud --custom-proxy user:pass@host:8080
```
...
```bash
penguin-browser -s <sessionId> -e "<code>"
```

The `-s` flag specifies a session ID (required). Get one with `penguin-browser session new`. Use the same session to persist state across commands.
...
penguin-browser -s 1 --timeout 120000 -e '...'

# Default for all -e/-f in this shell
export PENGUIN_BROWSER_EXEC_TIMEOUT=30000
penguin-browser -s 1 -e '...'
```

PENGUIN_BROWSER_EXEC_TIMEOUT is the default fallback. --timeout overrides it, and MCP clients can set the env var or pass a per-call timeout.
**Examples:**

```bash
# Navigate to a page
penguin-browser -s 1 -e 'state.page = await context.newPage(); await state.page.goto("https://example.com")'

# Click a button
penguin-browser -s 1 -e 'await state.page.click("button")'

# Get page title
penguin-browser -s 1 -e 'await state.page.title()'

# Take a screenshot
penguin-browser -s 1 -e 'await state.page.screenshot({ path: "/absolute/path/to/screenshot.png", scale: "css" })'

# Get accessibility snapshot
penguin-browser -s 1 -e 'await snapshot({ page: state.page })'

# Get accessibility snapshot for a specific iframe
penguin-browser -s 1 -e 'const frame = await state.page.locator("iframe").contentFrame(); await snapshot({ frame })'
```
...
```bash
penguin-browser logfile  # prints the log file path
# typically: ~/.penguin-browser/relay-server.log
```

The relay log contains logs from the extension, MCP and WS server. A separate CDP JSONL log is created alongside it (see `penguin-browser logfile`) with all CDP commands/responses and events, with long strings truncated. Both files are recreated every time the server starts. For debugging internal penguin-browser errors, read these files with grep/rg to find relevant lines.

Example: summarize CDP traffic counts by direction + method:

```bash
jq -r '.direction + "\t" + (.message.method // "response")' ~/.penguin-browser/cdp.jsonl | uniq -c
```

If you find a bug, capture a minimal reproduction and the relevant local log paths. Ask the user where this private project tracks issues, and get confirmation before creating anything externally.
...
For automated screen recording without a manual extension click, prefer the managed browser command. It loads the bundled extension and derives the correct extension ID for Chrome's tab-capture flags:

```bash
penguin-browser browser start
```

When launching an existing Chrome installation manually, copy the extension ID from `chrome://extensions` and substitute it below:

```bash
export PENGUIN_BROWSER_EXTENSION_ID=replace-with-the-extension-id

# macOS/Linux (adapt the Chrome executable for your platform)
google-chrome --profile-directory=Default --allowlisted-extension-id="$PENGUIN_BROWSER_EXTENSION_ID" --auto-accept-this-tab-capture
```

```bat
# Windows (cmd)
set PENGUIN_BROWSER_EXTENSION_ID=replace-with-the-extension-id
start chrome.exe --profile-directory=Default --allowlisted-extension-id=%PENGUIN_BROWSER_EXTENSION_ID% --auto-accept-this-tab-capture
```

You can collaborate with the user - they can help with captchas, difficult elements, or reproducing bugs.

**Direct CDP mode (no extension needed):** Penguin Browser can connect directly to Chrome's DevTools Protocol, bypassing the extension. This is useful in CI, Docker, headless environments, when Chrome has `--remote-debugging-port=9222`, or with cloud browser providers (e.g. `wss://xxx.cdp.browser-use.com`). If the user provides a CDP URL, set `PENGUIN_BROWSER_DIRECT` in the MCP client config:
...
`PENGUIN_BROWSER_DIRECT` accepts `1` (auto-discover Chrome on port 9222), a `ws://` or `wss://` endpoint (including cloud browser providers), or `host:port`. Screen recording is not available in direct CDP mode since it relies on the extension's `chrome.tabCapture` API.
...
- The **directory where `penguin-browser` CLI was invoked** (the session's cwd)
- `/tmp`
...
- **CDP sessions**: use `getCDPSession({ page: state.page })` not `state.page.context().newCDPSession()` - NEVER use `newCDPSession()` method, it doesn't work through penguin-browser relay
...
penguin-browser -s 1 -e 'await state.page.locator(`[id="_r_a_"]`).click()'
...
**9. Not using penguin-browser for JS-rendered sites**
Do NOT waste context trying webfetch, curl, or Playwright CLI screenshots on SPAs (Instagram, Twitter, etc.). These return empty HTML shells. Use penguin-browser directly:
...
**Pages are shared, state is not.** `context.pages()` returns all browser tabs with penguin-browser enabled — shared across all sessions. Multiple agents see the same tabs. If another agent navigates or closes a page you're using, you'll be affected. To avoid interference, **get your own page**.
...
if (pages.length === 0) throw new Error('No myapp.com page found. Ask user to enable penguin-browser on it.')
...
Use this for explicitly page-scoped cookie access. Browser-context cookie APIs (`context.cookies()`,
`context.addCookies()`, and `context.clearCookies()`) are also supported in extension mode, but are
restricted to URLs of pages explicitly connected to penguin-browser. In particular,
`context.clearCookies()` deletes only cookies visible to those authorized page URLs; it never clears
the rest of the user's Chrome profile.
...
**inspectPinnedElement** - inspect a Penguin Browser pinned element and print the element `outerHTML` plus React component info when available. Used by the in-page toolbar and right-click copy flow.

```js
await inspectPinnedElement('https://example.com', 'globalThis.penguinBrowserPinnedElem1')
```

**getStylesForLocator** - inspect CSS styles applied to an element, like browser DevTools "Styles" panel. Useful for debugging styling issues, finding where a CSS property is defined (file:line), and checking inherited styles. Returns selector, source location, and declarations for each matching rule. ALWAYS read the bundled MCP resource `penguin-browser://resources/styles-api.md` first with `resources/read`.
...
**createEditor** - view and live-edit page scripts and CSS at runtime. Edits are in-memory (persist until reload). Useful for testing quick fixes, searching page scripts with grep, and toggling debug flags. ALWAYS read the bundled MCP resource `penguin-browser://resources/editor-api.md` first with `resources/read`.
...
const recordingState = await recording.isRecording({ page: state.page })
await recording.cancel({ page: state.page }) // cancel without saving
```

**ghostCursor.show / ghostCursor.hide** - the ghost cursor overlay is always on: the extension injects it on every Penguin Browser-attached tab and it stays visible at the last spot Playwright clicked or moved. These methods only matter if you want to change the cursor style or temporarily hide it:
