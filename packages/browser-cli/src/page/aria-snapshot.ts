...
function getA11yClientCode(): string {
  ...
  const a11yClientPath = distPath('a11y-client.js')
...
export async function getAriaSnapshot({
  ...
  const resolvedPage = resolvedFrame?.page() ?? page

  // Reuse Playwright's already-attached OOPIF session when the frame owns one.
  // Target.attachToTarget cannot create a usable nested session through the relay,
  // and matching iframe targets by URL is ambiguous when URLs repeat.
  const frameSession = resolvedFrame ? await getCDPSessionForFrame({ frame: resolvedFrame }) : null
  const session = frameSession || cdp || (await getCDPSessionForPage({ page: resolvedPage }))
  const ownsSession = Boolean(frameSession) || !cdp
  ...
  try {
    ...
    const axParams = !frameSession && frameId ? { frameId } : undefined
...
export async function hideAriaRefLabels({ page }: { page: Page }): Promise<void> {
  await page.evaluate(() => {
    ...
    if (a11y) {
      ...
      const timerKey = '__penguinBrowser_labels_timer__'
      if (win[timerKey]) {
        win.clearTimeout(win[timerKey])
        win[timerKey] = null
      }
      doc.getElementById('__penguinBrowser_labels__')?.remove()
    }
  })
}
