...
//   1. PENGUIN_BROWSER_API_KEY env var → sent as x-api-key header (for CI/VPS/headless)
//   2. PENGUIN_BROWSER_CLOUD_TOKEN env var → sent as Authorization: Bearer (for CI with session tokens)
//   3. ~/.penguin-browser/auth.json file → saved by `cloud login` device flow

import fs from 'node:fs'
import os from 'node:os'
import path from 'node:path'

const DEFAULT_BASE_URL = 'https://private-project.invalid'
const AUTH_FILE = path.join(os.homedir(), '.penguin-browser', 'auth.json')

/** Build a private-project.invalid/live URL from the exact CDP WebSocket URL.
...
export function loadCloudAuth(): CloudAuth | null {
  ...
  const apiKey = process.env.PENGUIN_BROWSER_API_KEY
  if (apiKey) {
    return { token: apiKey, baseUrl: process.env.PENGUIN_BROWSER_CLOUD_URL || DEFAULT_BASE_URL, isApiKey: true }
  }
  // Session token env var (for CI with device flow tokens)
  const envToken = process.env.PENGUIN_BROWSER_CLOUD_TOKEN
  if (envToken) {
    return { token: envToken, baseUrl: process.env.PENGUIN_BROWSER_CLOUD_URL || DEFAULT_BASE_URL }
  }
  try {
...
export class CloudClient {
  ...
  private async request<T>(
    ...
    if (response.status === 401) {
      throw new Error('Cloud auth expired or invalid. Run `penguin-browser cloud login` or set PENGUIN_BROWSER_API_KEY.')
    }
