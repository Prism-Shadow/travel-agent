...
export function resolveBrowserExecutablePath({
  ...
  const envPath = env.PENGUIN_BROWSER_BROWSER_PATH?.trim()
  if (envPath) {
    const resolvedEnvPath = expandHomeDirectory({ filePath: envPath, homeDir })
    if (!existsSync(resolvedEnvPath)) {
      throw new Error(`PENGUIN_BROWSER_BROWSER_PATH does not exist: ${resolvedEnvPath}`)
    }
    return resolvedEnvPath
  }

  // Check Chrome downloaded by `penguin-browser install` first (highest priority)
  ...
  throw new Error(
    'Could not find a supported browser binary. Run `penguin-browser browser install` to download Chrome for Testing, or pass a binary path.' +
      `\n\nSearched paths:\n- ~/.penguin-browser/browsers/ (penguin-browser browser install)\n${searchedPathsText.join('\n')}`,
  )
}
