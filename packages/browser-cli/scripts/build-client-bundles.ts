 ...
 *
 * Each bundle uses a separate esbuild call (not multiple entrypoints in one call)
...
async function buildBundle(config: BundleConfig): Promise<void> {
  ...
  try {
    ...
    // Cleanup temporary entry file even when the bundle throws, so a failed build
    // does not leave `_<name>-entry-<pid>-<ts>.js` behind in dist/.
  ...
  if (!output) {
    throw new Error(`Failed to bundle ${config.name}: esbuild produced no output`)
  }
