...
- **Built-in administrator `traveler` starts with the fixed initial password `traveler-2026`** (`INITIAL_ADMIN_CREDENTIALS` in the API contract; `PENGUIN_SEED_ADMIN_PASSWORD` overrides it for tests, e2e and hardened deployments). The pair is public by decision: the login page shows it, and web mode prints a framed reminder on every start until the password is changed. Desktop mode seeds the same default but signs its window in by shell token and prints nothing. Change the password promptly on any server that is not loopback-only.
- **Legacy administrator upgrade**: startup renames the privileged legacy `admin` to `traveler` atomically with its existing password and data, invalidating its old login sessions. Sign in again as `traveler`. An existing `traveler` account refuses startup without merging accounts; resolve the identity conflict before retrying. Back up the data root while the server is stopped before upgrading. See the [account continuity contract](SPEC.md#what-it-owns).
