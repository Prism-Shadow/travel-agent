...
/** A pre-upgrade data root: the PenguinHarness `admin`, before `previous_user_id` existed. */
async function seedLegacy(initial = false) {
  const source = "admin";
  const db = new sqlite.DatabaseSync(dbPath);
  db.exec(SCHEMA_SQL);
  db.exec("ALTER TABLE users DROP COLUMN previous_user_id");
...
describe("traveler administrator upgrade", () => {
  it.each([false, true])("preserves account data and password (initial=%s)", async (initial) => {
    const before = await seedLegacy(initial);
    ...
    try {
      ...
      expect(db.prepare("SELECT user_id FROM auth_sessions").all()).toEqual([{ user_id: "alice" }]);
      expect(
      ...
      const old = await app.request("/api/auth/login", {
        ...
        body: JSON.stringify({ userId: "admin", password: "existing-password" }),
      });
      expect(old.status).toBe(401);
      expect(
        (await api.post("/api/admin/users", { userId: "admin", password: "another-password" }))
          .status,
      ).toBe(400);
      // Only the retired name is reserved; nothing else about naming changed.
      expect(
        (await api.post("/api/admin/users", { userId: "travel", password: "another-password" }))
          .status,
      ).toBe(201);
      expect(
        (
    ...
    try {
      expect(again.prepare("SELECT COUNT(*) AS n FROM users").get()?.n).toBe(3);
      expect(
  ...
  it("refuses an occupied traveler target without changing either account", async () => {
    await seedLegacy();
    const before = new sqlite.DatabaseSync(dbPath);
    before
      .prepare(
        "INSERT INTO users (user_id, password_hash, is_admin, password_is_initial, created_at) VALUES ('traveler', 'unrelated-hash', 0, 0, ?)",
      )
      .run(NOW);
    before.close();
    expect(() => openDatabase(dbPath)).toThrow("traveler account already exists");
    const after = new sqlite.DatabaseSync(dbPath);
    try {
      expect(
        after.prepare("SELECT password_hash FROM users WHERE user_id='traveler'").get()
          ?.password_hash,
      ).toBe("unrelated-hash");
      expect(
      ...
      expect(
        after.prepare("SELECT previous_user_id FROM users WHERE user_id='admin'").get()
          ?.previous_user_id,
      ).toBeNull();
    } finally {
      after.close();
  ...
  it("leaves a non-privileged admin account and its data untouched", async () => {
    await seedLegacy();
    const before = new sqlite.DatabaseSync(dbPath);
    before.exec("UPDATE users SET is_admin = 0 WHERE user_id = 'admin'");
    before.close();
  ...
  it("is idempotent: a migrated root keeps its original marker on every later open", async () => {
    await seedLegacy();
    openDatabase(dbPath).close();
    const again = openDatabase(dbPath);
    try {
      expect(
        again.prepare("SELECT user_id, previous_user_id FROM users WHERE is_admin=1").get(),
      ).toEqual({ user_id: "traveler", previous_user_id: "admin" });
