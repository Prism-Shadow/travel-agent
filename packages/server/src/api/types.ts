...
/**
 * Trip identity — the four things a traveller states before anything can be searched.
 *
 * These shapes are the product's own, not the engine's. They were prompt scaffolding on the
 * draft screen before the Trip existed (web `trip-constraints.ts`); they are now the identity
 * of a container that outlives any one conversation, so they live here, where both the server
...
/**
 * Price tiers rather than a number: people cannot state a numeric ceiling before seeing what
 * the options cost. The tier shapes the representative set; it is never a transaction gate.
 */
...
export interface TripSummary {
  ...
  projectId: string;
  name: string;
  /** Free text, possibly several places; "" = not set yet. */
  destination: string;
  when: TripWhen | null;
  who: TripWho | null;
...
export type TripPatchRequest = TripCreateRequest;
...
/** Attach / move / detach a conversation. `tripId: null` detaches it to floating. */
export interface TripAttachRequest {
  tripId: string | null;
}
...
export interface SessionInfo {
  ...
  workspace: string;
  /**
   * Owning Trip, or null for a floating ("scratch") conversation. Membership is mutable and
   * is deliberately not the workspace: attaching, moving or detaching a conversation never
   * changes where its files or its memory live.
   */
  tripId: string | null;
  approvalMode: ApprovalMode;
...
export interface SessionCreateRequest {
  ...
  workspace?: string;
  /** Create the Session already attached to this Trip; omit for a floating conversation. */
  tripId?: string;
