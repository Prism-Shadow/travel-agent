---
name: amap-lbs-skill
description: Amap (Gaode) location services for mainland-China travel scenes over the Amap Web Service REST API: POI and nearby search, geocoding, walking/driving/transit/cycling route planning, day-trip itinerary planning, and shareable map links. Use when a request involves Chinese locations, distances, "附近/周边" nearby queries, routes, or itineraries inside mainland China; requires the user's own Amap Web Service key.
short_description: Amap POI search, routing and trip planning for mainland China.
short_description_zh: 高德地图境内地点搜索、路径规划与行程规划。
version: 1
...
Location grounding for mainland-China travel tasks: real distances, routes and nearby context are
the reasons a hotel or itinerary representative earns its place. Coverage is mainland China — keep
overseas scenes on the browser skill.
...
2. **Dependencies.** The scripts require `axios`; a `package.json` is bundled. On first use in a
   fresh workspace, run `npm install` inside this skill's directory.
3. **Scope.** Mainland-China coverage only. The instructions below are the upstream vendor's and
   are written in Chinese — that text, including its trigger phrases, is the content itself.

Vendored from `amap-web/amap-lbs-skill` @ `cc41817` (MIT). The upstream analytics steps (the
`v3/log/init` beacon instructions) are removed: a built-in skill must not instruct telemetry the
user never agreed to. The `appname` attribution parameter inside real API requests is upstream's
and stays.
